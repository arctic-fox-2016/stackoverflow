...
router.post('/questions', function(req,res,next){
  ...
  let newQuestion = new Questions({user:req.body.username,title: req.body.title, content: req.body.content}).save(function(err,result){
    if(err){
      console.log(err)
    }
    res.json(result)
...
router.post('/answers/:q_id', function(req,res,next){
  console.log("masuklho")
  Users.findOne({username: req.body.username}, function(err1,result1){
    let newAnswer = new Answers({username:req.body.username, title: req.body.title, content: req.body.content}).save(function(err2,result2){
      if(err2){
        console.log(err2)
      }
